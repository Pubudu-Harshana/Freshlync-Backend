"""
Freshlync Demand Forecasting: Model Comparison Study
====================================================

This script evaluates and compares multiple modeling approaches for demand forecasting:
1. Moving Average (Baseline)
2. ARIMA (Statistical Time Series)
3. Prophet (Additive Seasonality Model)
4. XGBoost (Gradient Boosted Decision Trees)

It loads sample orders, runs time-series cross-validation splits, computes evaluation metrics 
(MAE, RMSE, MAPE, R2) for each model, and saves comparison charts and forecasts to the outputs folder.
"""
...
# ── Publication-quality chart helpers (used by generate_charts) ──────────────
_CAT_COLORS = {
    'fish':      '#0072B2',
    'meat':      '#D55E00',
    'vegetable': '#009E73',
...
def _pub_lims(arrays, margin=_MARGIN):
...
def _draw_avp_panel(ax, df, name):
    """Draw one Actual vs Predicted panel with metrics, equal aspect, y=x line."""
    actual    = df['actual'].values.astype(float)
    predicted = df['predicted'].values.astype(float)
    cats      = df['category'].values if 'category' in df.columns else np.full(len(actual), 'all')
    ...
    lo, hi = _pub_lims([actual, predicted])
    ...
    ax.text(0.04, 0.97, annot,
            ...
            bbox=dict(boxstyle='round,pad=0.4', facecolor='white',
                      edgecolor=_SPINE_COLOR, linewidth=0.8, alpha=0.88),
            zorder=5)

    ax.set_title(f'{name}: Actual vs Predicted',
                 fontsize=11, fontfamily=_FONT,
                 fontweight='semibold', color='#111111', pad=8)
    ax.set_xlabel('Actual (units)', fontsize=9,
                  fontfamily=_FONT, color='#333333', labelpad=5)
    ax.set_ylabel('Predicted (units)', fontsize=9,
                  fontfamily=_FONT, color='#333333', labelpad=5)
    _pub_style(ax)
    return r2, rmse, mae
...
def generate_charts(results_dict, metrics_df):
    ...
    # 2. MAPE comparison
    colors = ['tomato' if model == 'ARIMA' else 'teal' for model in metrics_df['Model']]
    metrics_df.set_index('Model')['MAPE'].plot(kind='bar', color=colors, figsize=(8, 5), edgecolor='black')
    ...
    # 3. Publication-ready Actual vs Predicted  ── 2x2 panel ──────────────────
    models_list = list(results_dict.items())
    n_models    = min(len(models_list), 4)
    nrows       = (n_models + 1) // 2
    ...
    for i, (name, df) in enumerate(models_list[:4]):
        _draw_avp_panel(axes[i], df, name)

    # Hide unused panels
    for j in range(n_models, len(axes)):
        axes[j].set_visible(False)

    # Shared legend (categories + ideal line)
    ...
    fig.savefig(os.path.join(CHARTS_DIR, 'actual_vs_predicted_all.png'),
                dpi=_DPI, bbox_inches='tight')
    plt.close(fig)
    print(f"  Saved publication-ready actual_vs_predicted_all.png")
...
def main():
    ...
    if PROPHET_AVAILABLE:
        ...
        print("Prophet is not installed. Skipping Prophet baseline.")
